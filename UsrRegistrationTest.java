...
public class UsrRegistrationTest {
	...
	static String userPassword = ("^(?=.*[0-9])" + "(?=.*[a-z])(?=.*[A-Z])" + "(?=.*[@#$%^&+=])" + "(?=\\S+$).{8,20}$");

	// Validate First Name using Lambda Function
	public static ValidateLastName validateFirstName = (String firstName) -> {
	...
	public static ValidateMobileNumber validateMobileNumber = (String mobileNumber) -> {
		...
		try {
			if (matcher.matches()) {
				...
				throw new UserRegistrationMain("Enter Valid MobileNumber",
						UserRegistrationMain.ExpectedType.MOBILENUMBER);
			}
		} catch (UserRegistrationMain e) {
			System.out.println(e);
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		try {
			validateFirstName.validateLastName(firstName);
		} catch (UserRegistrationMain e) {
			e.printStackTrace();
		}
